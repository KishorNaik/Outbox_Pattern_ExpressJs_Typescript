...
import { IServiceHandlerVoidAsync, Result, ResultError, sealed, Service, VoidResult, tryCatchResultAsync, ResultFactory, VOID_RESULT, StatusCodes, Container, StatusEnum, BoolEnum,  } from "@kishornaik/utils";
...
export interface ICreateOutboxDbService extends IServiceHandlerVoidAsync<ICreateOutboxDbServiceParameters>{
...
export class CreateOutboxDbService implements ICreateOutboxDbService {
  ...
  public handleAsync(params: ICreateOutboxDbServiceParameters): Promise<Result<VoidResult, ResultError>> {
    return tryCatchResultAsync(async ()=>{
      ...
      if(result.isErr())
        return ResultFactory.errorInstance(result.error);
