import { IServiceHandlerAsync, JsonString, Result, ResultError, ResultFactory, sealed, Service, StatusCodes, tryCatchResultAsync } from "@kishornaik/utils";
import { WelcomeUserRequestDto } from "../../../../contracts";

export interface IMapWelcomeUserEmailService extends IServiceHandlerAsync<JsonString,WelcomeUserRequestDto>{
...
export class MapWelcomeUserEmailService implements IMapWelcomeUserEmailService{
  public handleAsync(params: JsonString): Promise<Result<WelcomeUserRequestDto, ResultError>> {
    return tryCatchResultAsync(async ()=>{
      ...
      if(!data)
        return ResultFactory.error(StatusCodes.BAD_REQUEST, 'Data is required');
